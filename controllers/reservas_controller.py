...
def _check_ownership(db: Session, reserva, usuario: Usuario):
    ...
    if rol.nombre == "Propietario":
        ...
        # Cancha.id_usuario es el propietario/dueño de la cancha
        if cancha and cancha.id_usuario != usuario.id_usuario:
...
def listar(db: Session, usuario: Usuario, skip: int = 0, limit: int = 100):
    ...
    if rol.nombre == "Jugador":
        ...
        from models.modelos import Reserva
        return db.query(Reserva).join(Cancha).filter(Cancha.id_usuario == usuario.id_usuario).offset(skip).limit(limit).all()
...
def crear(db: Session, datos: ReservaCreate, usuario: Usuario):
    ...
    # HU72: Generar notificación automática al crear reserva
...
def actualizar(db: Session, item_id: int, datos: ReservaUpdate, usuario: Usuario):
    ...
    if old_estado != updated_obj.estado:
        noti = Notificacion(
            ...
            mensaje=f"El estado de tu reserva #{updated_obj.id_reserva} cambió a: {updated_obj.estado}.",
            leida=False,
        )
        db.add(noti)
        db.commit()
