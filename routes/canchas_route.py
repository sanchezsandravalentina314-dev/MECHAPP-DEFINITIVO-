...
def crear_canchas(datos: CanchaCreate, db: Session = Depends(get_db)):
    return controller.crear(db, datos)

@router.put("/{item_id}", response_model=CanchaResponse)
def actualizar_canchas(item_id: int, datos: CanchaUpdate, db: Session = Depends(get_db)):
    return controller.actualizar(db, item_id, datos)

@router.delete("/{item_id}")
def eliminar_canchas(item_id: int, db: Session = Depends(get_db)):
